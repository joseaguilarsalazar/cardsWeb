...
from .models import Card, SafetyQuestion, SafetyAnswer
...
def home_view(request):
    ...
    # Filtramos las entradas de cada uno ordenadas de la más reciente a la más antigua
    mis_entradas = Card.objects.filter(owner=usuario_actual).order_by('-fecha_redaccion')
    
    if pareja:
        entradas_pareja = Card.objects.filter(owner=pareja).order_by('-fecha_redaccion')
    else:
        entradas_pareja = []
